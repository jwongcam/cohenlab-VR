function code =BH_shapingFixed_fake
...
if vr.position(2) > endPosition % test if the animal is at the end of the track
    ...
    if (vr.rig.isAcquiring)
        ...
        vr.reward_pos(vr.trialNumber+1)=(endPosition).*0.4;
    end
end
...
if (vr.rig.isAcquiring)
    ...
    fwrite(vr.fid, [timestamp vr.give_water vr.position vr.lickVoltage(end) vr.trialNumber],'double');
