 function code = Environment_Switch_BH_Zap
...
if isequal(vr.exper.movementFunction, @runFromRecording)
    ...
    vr.reward_pos=[repmat(vr.endPosition*0.8,1,vr.World_change_lap-1) repmat(vr.endPosition*0.8,1,1e4)];
...
% Open Blue
while vr.rig.isAcquiring == 0
    vr.stim_lap=vr.stim_lap+vr.trialNumber-1;
end
...
fwrite(vr.fid, [timestamp vr.currentWorld vr.rig.latestEncoderReading ...
       vr.position vr.trialNumber LickVoltage vr.rig.isAcquiring vr.reward_given(vr.trialNumber) vr.rig.BlueOn],'double');
